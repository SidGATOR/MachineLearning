...
In this project, we will use deep neural networks and convolutional neural networks to create an application or program that prints numbers it observes in from images it is given. First, we will design and test a model architecture that can identify sequences of digits in an image. Next, we will train that model so it can decode sequences of digits from natural images by using the [Street View House Numbers (SVHN) dataset](http://ufldl.stanford.edu/housenumbers/). Future work:After the model is properly trained, we will then test your model using a live camera application (optional) or program on newly-captured images. 
