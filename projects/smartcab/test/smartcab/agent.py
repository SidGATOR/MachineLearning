...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.50, decay_rate = 0.01):
    ...
    def get_maxQ(self, state):
        ...
        if state in self.Q:
            tmp = []
            for x,y in self.Q[state].items():
                tmp.append((y,x))
            tmp = sorted(tmp)[::-1]
            #tmp = max(tmp)
            print "Max Q: {:}" .format(tmp)
    ...
    def createQ(self, state):
        ...
        if self.learning:
            ...
            if state not in self.Q:
                self.Q[state] = {action_list[0] : 0.0, action_list[1] : 0.0, action_list[2] : 0.0, action_list[3]  : 0.0}
        """
        return
...
def run():
    ...
    learning = True
    agent = env.create_agent(LearningAgent,learning = True, alpha = 0.75, epsilon = 1.0,decay_rate = 0.001)
    ...
    n_test = 100
    sim.run(n_test=n_test,tolerance=0.035)
